...
# Mise en forme -----------
pop <- pop %>% filter(DEP=="59") %>% group_by(COM) %>% summarise(M2ans = round(sum(P16_POP0002)/sum(P16_POP)*100,1),
...
shpNord <- merge(shpNord, pop,
                 by.x ="insee", by.y="COM", all.x=T)

shpNord <- cartogram_cont(shpNord,"pop" , itermax=4)

ggplot() + 
  geom_sf(data=shpNord, aes(fill=P75ans), color ="transparent")+
